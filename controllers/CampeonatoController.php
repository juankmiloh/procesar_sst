             ...
             */
                ...
                if(isset($_GET['idRegistro']) && $_GET['idRegistro'] != ""){
                    ...
                    try{
                        ...
                        echo "La fase no se puede editar";
                        return;
                    }
                }   else {
                    /**
                    * Si el campeonato ya tiene fases, primero se valida que la fase anterior 
                    * este totalmente terminada
                    */
                   $fasesAnteriores = \app\models\CampeonatoTieneFases::find()
                       ->where('camp_id = '. $_GET['camp_id'])
                       ->orderBy('ctf_id desc')
                       ->one();
                   if($fasesAnteriores != null){
                       $totalClasificados = $fasesAnteriores->ctf_cantidad_grupos * $fasesAnteriores->ctf_clasificados_grupo;
                       $resultados = \app\models\CampeonatoTieneResultados::find()
                              ->where('ctf_id = '.$fasesAnteriores->ctf_id." and ftr_clasifica = 1")
                               ->count();
                       if($totalClasificados != $resultados){
                           $continuar = false;
                           echo "La fase anterior no ha sido terminada. Hay ".$resultados." clasificados de ".$totalClasificados.". No se puede crear otra fase";
                       }
                   } 
                }   
