...
class HelperController extends Controller
{
    ...
    public function actionBuscarprogramacion(){
        if(isset($_GET['fase'])){
            ...
            $fechas = \app\models\FaseTieneEncuentros::find()
                    ->leftJoin('equipo e1', 'equi_id_1 = e1.equi_id')
                    ->leftJoin('entidad en1', 'en1.ent_id = e1.ent_id')
                    ->leftJoin('departamentos dpt1', 'dpt1.dptos_id = en1.ent_dpto')
                    ->leftJoin('equipo e2', 'equi_id_2 = e2.equi_id')
                    ...
                    ->leftJoin('departamentos dpt2', 'dpt2.dptos_id = en2.ent_dpto')
                    ->leftJoin('escenario esc', 'esc.esc_id = fase_tiene_encuentros.esc_id')
                    ->where('ctf_id = '.$idFase.' and tfs_publicar = 1')
                    ->select('tfs_fecha_hora')
                    ->orderBy('tfs_fecha_hora asc')
                    ->asArray()->all();
            
            $array = [$encuentros, $fechas];
    ...
    public function actionBuscarposiciones(){
        ...
        if(isset($_GET['fase'])){
            ...
            $posiciones = \app\models\CampeonatoTieneResultados::find()
                    ...
                    ->leftJoin('campeonato_tiene_fases ctf', 'ctf.ctf_id = campeonato_tiene_resultados.ctf_id')
                    ->leftJoin('departamentos dpto', 'dpto.dptos_id = e.ent_dpto')
                    ->where('campeonato_tiene_resultados.ctf_id = '.$idFase)
    ...
    public function actionBuscarresultados(){
        ...
        if(isset($_GET['fase'])){
            $idFase = $_GET['fase'];
            $sql = "SELECT fte.fts_id, fte.tfs_archivo_resultado, e.ent_nombre as equip1, e3.ent_nombre as equip2, fte.tfs_gf_1, fte.tfs_gf_2, fte.tfs_fecha_hora, fte.fts_grupo, e1.equi_id eq1, e2.equi_id eq2, fte.fts_id, dpto1.dptos_name dp1, dpto2.dptos_name dp2
                FROM fase_tiene_encuentros fte 
                  ...
                  WHERE fte.ctf_id = ".$idFase." AND fte.tfs_fecha_hora < now() order by fte.tfs_fecha_hora asc";
