...
class Usuario extends \yii\db\ActiveRecord implements \yii\web\IdentityInterface
{
    ...
    public function getRol(){
        $rol = UsuarioTieneRol::find()->where('usu_id = '.$this->getPrimaryKey() )->orderBy('utr_id asc')->one();
        if($rol != null){
            return $rol->getRol()->one()->rol_nombre;
        } else {
            return "SIN ROL";
        }
    }
