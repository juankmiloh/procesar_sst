...
<body class="background_images">
            ...
            <div class="container-fluid">                
                <div class="row gutter">
                    ...
                    <div class="col-md-2 col-sm-6 col-xs-12">
                        <button type="button" class="btn teal-bg btn-block" onclick="" data-toggle='modal' data-target='#modalAsignarNumero'><i class="icon-circle-plus icon-left"></i><?= Yii::t('app', "ASIGNAR_NUMERO") ?></button>
                    </div>                                         
                    <div class="col-md-2 col-sm-6 col-xs-12">
    ...
    <input id="listarFases" type="hidden" value="<?= Url::toRoute(['atletismo/listarfases']); ?>">
...
<!--                        <div class="form-group">
    ...
    <!-- MODAL ASIGNAR NUMERO -->
    <div class="modal fade" id="modalAsignarNumero" role="dialog" aria-labelledby='modalForm2'>
        <div class="modal-dialog" role="document" style="width: 70%;">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><?php echo Yii::t('app', "ASIGNAR_NUMERO") ?></h4>
                </div>
                <form id="formularioAsignarNum" onsubmit="return false;">
                    <div class="modal-body">
                    <form>
                        <div class="form-group">                        
                            <div class="row gutter"> 
                            	<div class="col-md-12">
                            		...
                            		<label class="control-label"><?= Yii::t('app', "EVENTOs_LOWWER") ?></label>
                                    <?php
                                        echo Html::dropDownList('eve_id_num_dep','eve_id_num_dep',$eventos,['prompt'=>Yii::t("app", "SELECCIONE"), 'class'=>'form-control','style' => 'width: 375px', 'id' => 'eve_id_num_dep', 'tabindex' => 1]);
                                    ?>
                                    </center>
                            	</div>                           
                                <div class="col-md-12">
                                    <div class="col-md-6 col-xs-12">
                                        <label class="control-label"><?php echo Yii::t('app', "CATEGORIA") ?></label>
                                        <?php echo Html::dropDownList('cat_id','cat_id', $categorias, ['prompt'=>Yii::t("app", "SELECCIONE"), 'class'=>'form-control filter', 'id'=>'cat-id']); ?>
                                    </div>
                                    <div class="col-md-6 col-xs-12">
                                        <label class="control-label"><?php echo Yii::t('app', "GENERO") ?></label>
                                        <?php echo Html::dropDownList('gen_id','gen_id', $genero, ['prompt'=>Yii::t("app", "SELECCIONE"), 'class'=>'form-control', 'id'=>'gen-id']); ?>
                                    </div>
                                </div>                            
                            </div>                        
                        </div><hr><br>
                        <table id="list-jugadores" class="display" cellspacing="0" width="90%">
                            <thead> 
                                <tr>
                                    <th><?php echo Yii::t('app', "DEPARTAMENTO") ?></th>
                                    <th><?php echo Yii::t('app', "CIUDAD") ?></th>
                                    <th><?php echo Yii::t('app', "INSTITUCION/EQUIPO") ?></th>
                                    <th><?php echo Yii::t('app', "NOMBRE") ?></th>
                                    <th><?php echo Yii::t('app', "DOCUMENTO") ?></th>
                                    <th><?php echo Yii::t('app', "NUMERO") ?></th>
                                    <th><?php echo Yii::t('app', "OPCIONES") ?></th>
                                </tr>
                                <tr>
                                    <th><?php echo Yii::t('app', "DEPARTAMENTO") ?></th>
                                    <th><?php echo Yii::t('app', "CIUDAD") ?></th>
                    ...
                    <input id="tipoReporte" type="hidden" name="tipRep">
                    <div class="modal-footer">
                        <input id="urlGeneracionReporte" type="hidden" value="<?php echo Url::toRoute(['atletismo/generarreporte'])?>">
                        <label class="control-label" style="font-weight: bold;"><?php echo Yii::t('app', "GENERAR_REPORTE")?>: </label>
                        <button class="btn btn-success btn-sm" onclick="$('#tipoReporte').val(1),generarReporte();"><i class="icon-paper icon-left"></i><?php echo Yii::t('app',"EXCEL") ?></button>
                        <button class="btn btn-danger btn-sm" onclick="$('#tipoReporte').val(2),generarReporte();"><i class="icon-file-pdf icon-left"></i><?php echo Yii::t('app',"PDF") ?></button><hr>
                        <button type="button" class="btn btn-default" data-dismiss="modal"><i class="icon-circle-with-cross icon-left"></i> <?=Yii::t('app', "CERRAR")?></button>                    
                    </div>
                </form>
            </div>
    ...
    <!-- FIN MODAL ASIGNAR NUMERO -->        

    <input id="urlAsignarNum" type="hidden" value="<?php echo Url::toRoute(['atletismo/asignarnumero']) ?>"> 
    <input id="urlMunicipiosJugadores" type="hidden" value="<?php echo Url::toRoute(['atletismo/listmunicipios']) ?>">
    <input id="urlLimpiarNumero" type="hidden" value="<?php echo Url::toRoute(['atletismo/limpiarrnumero']) ?>">
...
<script>
    ...
    var DEBE_INGRESAR_NUMERO = '<?php echo Yii::t('app', "DEBE_INGRESAR_NUMERO") ?>';
    var NUMERO_YA_ASIGNADO = '<?php echo Yii::t('app', "NUMERO_YA_ASIGNADO") ?>';
    var NUMERO_ASIGNADO_EXITO = '<?php echo Yii::t('app',"NUMERO_ASIGNADO_EXITO")?>';
    var NUMERO_BORRADO_EXITO = '<?php echo Yii::t('app', "NUMERO_BORRADO_EXITO") ?>';
    ...
    $(document).ready(function () {
        ...
        var table = $('#list-datos').DataTable({
            ...
            "serverSide": true,
            "order": [[ 0, "desc" ]],
            "ajax": "<?php echo Url::toRoute(['atletismo/data', 'idDeporte' => $idDeporte]); ?>", 
            "language": {
        ...
        $('#list-datos thead tr:eq(1) th').each( function () {
            ...
            if(column > 0 && column < ($('#list-datos').columnCount() - 1)){ 
                ...
                if(column == 1){//Construye una lista 
                    <?php
                        $html = Html::dropDownList('eve_id2', 'eve_id2', $eventos, ['prompt' => Yii::t('app', ''), 'class' => 'form-control filter', 'id' => 'eve_id2', 'name' => 'eve_id2', 'style'=>'width: 190px']);
                        $html = preg_replace( "/\r|\n/", "", $html );;
                        $data = "";
                    ?>
...
//                    <?php
                        $html = Html::dropDownList('prom_id2', 'prom_id2', $categorias, ['prompt' => Yii::t('app', ''), 'class' => 'form-control filter', 'id' => 'prom_id2', 'name' => 'prom_id2']);
                        $html = preg_replace( "/\r|\n/", "", $html );;
                        $data = "";
                    ?>//
//                    $(this).html( '<?php echo $html; ?>' );
...
//                } 
                    ...
                    <?php
                        $html = Html::dropDownList('prueb_id2', 'prueb_id2', $pruebas, ['prompt' => Yii::t('app', ''), 'class' => 'form-control filter', 'id' => 'prueb_id2', 'name' => 'prueb_id2', 'style'=>'width: 190px']);
                        $html = preg_replace( "/\r|\n/", "", $html );;
                        $data = "";
                    ?>
                    $(this).html( '<?php echo $html; ?>' );
        ...
        var table1 = $('#list-jugadores').DataTable({
            ...
            "serverSide": true,
            "order": [[ 0, "desc" ]],
            "ajax": "<?php echo Url::toRoute(['atletismo/datadeportistas']); ?>",
            "language": {
                "lengthMenu": "<?php echo Yii::t("app", "DISPLAY_RESULT") ?>",
                "zeroRecords": "<?php echo Yii::t("app", "NO_RESULT") ?>",
                "info": "<?php echo Yii::t("app", "RES_PAG") ?>",
        ...
        // Filtros columnas deportistas        
        column = 0;
        $('#list-jugadores thead tr:eq(1) th').each( function () {            
            //No agregue filtro para el registro ID
            if(column >= 0 && column < ($('#list-jugadores').columnCount())){ 
                
                if(column == 0){//Construye una lista 
                    <?php
                        $html = Html::dropDownList('dptos_id2', 'dptos_id2', $dptos, ['prompt' => Yii::t('app', 'TODO'), 'class' => 'form-control filter', 'id' => 'dptos_id2', 'name' => 'dptos_id2']);//, 'onchange'=>'listarMunicipios(this.value)'
                        $html = preg_replace( "/\r|\n/", "", $html );
                        $data = "";
                    ?>
                    $(this).html( '<?php echo $html; ?>' );
                    $('#dptos_id2').select2();
                }else if(column == 1){//Construye una lista 
                    <?php
                        $html = Html::dropDownList('municipios_id2', 'municipios_id2', $mcipios, ['prompt' => Yii::t('app', 'TODO'), 'class' => 'form-control filter', 'id' => 'municipios_id2', 'name' => 'municipios_id2']);
                        $html = preg_replace( "/\r|\n/", "", $html );
                        $data = "";
                    ?>
                    $(this).html( '<?php echo $html; ?>' );
                    ...
                    $(this).html( '<input class="form-control filter" name="'+name+'" style="height: 15px;" id="" type="text"  />' );
                }
            }
            column++;
        });
        ...
        table1.columns().every(function (index) {            
            ...
            $('#list-jugadores thead tr:eq(1) th:eq(' + index + ') input').on('keyup change', function () {
                if(this.value.length > 3 || this.value.length == 0){
                    table1.column($(this).parent().index() + ':visible')
                        .search(this.value).draw();
                }
...
<script src="../views/atletismo/js/atletismo.js"></script>