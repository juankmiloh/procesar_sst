...
class ProgramacionController extends Controller
{
                ...
                'rules' => [
                    [
                        'actions' => ['index', 'sorteo', 'encuentros', 'guardarsorteo', 
                            ...
                            'publicarprogramacion', 'consultarresultado', 'archivoresultados'], // add all actions to take guest to login page
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionArchivoresultados(){
        if ( 0 < $_FILES['file']['error'] ) {
            ...
            $ctf = \app\models\FaseTieneEncuentros::find()
                    ->where('fts_id = '.$_GET['idFaseTieneEncuentros'])
                    ->one();
            $ctf->tfs_archivo_resultado = $path . $_FILES['file']['name'];
            $ctf->save();
    ...
    public function actionVerresultados(){
        ...
        for($i = 0; $i < $totalTablas; $i++){
           ...
           $results = $connection->createCommand($sql)->queryAll();
            ...
            $fin = 0;
            $count = 0;
            for($dos = 0; $dos < 2; $dos++){ //Se realiza dos veces para asegurarnos que funcione en los casos de triple empate
                for($lk = 0; $lk < count($results); $lk++)
                {
                    if(isset($results[$lk + 1]['pu']) && ($results[$lk]['pu'] == $results[$lk + 1]['pu'])){
                        
                        if(($results[$lk]['gl'] > $results[$lk + 1]['gl'])){ //Desempate por juego limpio
                            $temp = $results[$lk];
                            $results[$lk] =  $results[$lk + 1];
                            $results[$lk + 1] = $temp;
                        } else if($results[$lk]['gl'] == $results[$lk + 1]['gl']){ // Si juego limpio estan iguales se desempata por diferencia de goles
                            $diff_goles_1 = $results[$lk]['gf'] - $results[$lk]['ge'];
                            $diff_goles_2 = $results[$lk+1]['gf'] - $results[$lk+1]['ge'];
                            
                            if($diff_goles_1 == $diff_goles_2){ //Si DIFF de goles estan iguales se desempata por mayor # de goles
                                if($results[$lk+1]['gf'] > $results[$lk]['gf']){
                                    $temp = $results[$lk];
                                    $results[$lk] =  $results[$lk + 1];
                                    $results[$lk + 1] = $temp;
                                }
                            } else {
                                if($diff_goles_2 > $diff_goles_1){
                                    $temp = $results[$lk];
                                    $results[$lk] =  $results[$lk + 1];
                                    $results[$lk + 1] = $temp;
                                }
    ...
    public function actionGetsucesos(){
        ...
        $sucesos = \app\models\EncuentrosTieneResultados::find()
                ...
                ->leftJoin('parametrizacion_tiene_sucesos pts', 'pts.pts_id = encuentros_tiene_resultados.pts_id')
                ->leftJoin('parametrizacion_tiene_sucesos pts2', 'pts2.pts_id = encuentros_tiene_resultados.etr_tiempo')
                ->where('fts_id = '.$fte." and equi_id_".$numEquipo." = ".$idEquipo)
                ->select("CONCAT(u.usu_apellidos , `u`.`usu_nombres`) as nombre, pts.pts_suceso, etr_minuto, pts2.pts_suceso as etr_tiempo, etr_id")
                ->orderBy('etr_minuto')
                ->asArray()
                ->all();
